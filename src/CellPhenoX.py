...
class CellPhenoX:
    def __init__(self, X, y, CV_repeats, outer_num_splits, inner_num_splits):
        ...
        self.param_grid = [{'max_features': ['sqrt', 'log2'],
                        'max_depth': [10, 20, 30],
    ...
    def split_data(self, train_outer_ix, test_outer_ix):
        X_train_outer, X_test_outer = self.X.iloc[train_outer_ix, :], self.X.iloc[test_outer_ix, :]
        y_train_outer, y_test_outer = self.y.iloc[train_outer_ix], self.y.iloc[test_outer_ix]
        ...
        return [X_train_outer, X_test_outer, X_train_inner, X_val_inner, y_train_inner, y_train_outer, y_test_outer, y_val_inner]
    ...
    def model_training_shap_val(self, outpath):
        ...
        for i, CV_repeat in enumerate(range(self.CV_repeats)):
            ...
            #Establish CV scheme
            CV = StratifiedKFold(n_splits=self.outer_num_splits, shuffle=True, random_state=self.random_states[i])
            ...
            for j, (train_outer_ix, test_outer_ix) in enumerate(zip(ix_training, ix_test)):
                #Verbose
                print('\n------ Fold Number:',j+1)
                X_train_outer, X_test_outer, X_train_inner, X_val_inner, y_train_inner, y_train_outer, y_test_outer, y_val_inner = self.split_data(train_outer_ix, test_outer_ix)
                ## Establish inner CV for parameter optimization #-#-#
                cv_inner = StratifiedKFold(n_splits=self.inner_num_splits, shuffle=True, random_state=1) #-#-#

                # Search to optimize hyperparameters
                #hp_start = time.time()
                model = RandomForestClassifier(random_state=10, class_weight='balanced')
                search = RandomizedSearchCV(model, self.param_grid, scoring='balanced_accuracy', cv=cv_inner, refit=True, n_jobs=1) #-#-#
                ...
                print("--- Validation Accuracy: ", val_accuracy, " - Validation AUROC: ", val_auc, " - Val AUPRC: ", val_prc)
                #pe_end = time.time()
            ...
            precision, recall, thresholds = precision_recall_curve(y_test_combined, y_prob_combined)
            ...
            best_model_repeat, best_score_repeat = max(model_pr_pairs, key=lambda x: x[1])
            ...
            axes[1].plot(recall, precision, lw=2, label=f'CV Repeat{i+1} (PRC = {prc_auc:.2f})')
        ...
        axes[2].hist(predicted_negative, label='Negative Class', bins=30, color="#f09102", alpha=0.5)
        axes[2].hist(predicted_positive, label='Positive Class', bins=30, color="#2d5269", alpha=0.5)
